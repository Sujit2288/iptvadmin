...
const Users: React.FC<UsersProps> = ({ users = [], packages = [], onRecharge, onDelete }) => {
  ...
  // Enhanced safety for search logic
  ...
  return (
    <div className="space-y-10 animate-in duration-500">
      <div className="bg-white p-12 rounded-[4rem] border border-slate-100 shadow-sm flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 w-full lg:w-auto">
          ...
          <div className="px-10 py-5 bg-blue-50 rounded-[2rem] border border-blue-100 flex items-center space-x-4">
            <span className="text-[11px] font-black text-blue-600 uppercase tracking-[0.2em]">Total Nodes</span>
                ...
                <tr key={user.id} className="hover:bg-slate-50/50 transition-all group">
                  <td className="px-14 py-10">
                    <div className="flex items-center space-x-10">
                      <div className={`w-20 h-20 rounded-[2.5rem] flex items-center justify-center text-white text-3xl font-black shadow-2xl transition-transform group-hover:scale-110 ${user.status === 'Active' ? 'bg-blue-600' : 'bg-slate-400'}`}>
                  ...
                  <td className="px-14 py-10">
                    <div className="flex items-center space-x-3">
                      ...
                      <span className={`text-[11px] font-black uppercase tracking-[0.2em] ${user.status === 'Active' ? 'text-emerald-600' : 'text-red-500'}`}>
                        {user?.status || 'UNKNOWN'}
                      </span>
                    </div>
                  </td>
      ...
      {isRechargeModalOpen && selectedUser && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-slate-900/95 backdrop-blur-2xl animate-in fade-in">
          <div className="bg-white rounded-[4rem] shadow-[0_0_200px_rgba(0,0,0,0.4)] w-full max-w-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300 border border-white/20">
            <div className="p-12 border-b border-slate-100 flex justify-between items-center bg-white rounded-t-[4rem]">
              <div>
                <h4 className="text-4xl font-black text-slate-800 tracking-tighter">Renew Subscription</h4>
                <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] mt-3">NODE: {selectedUser?.name || 'Unknown'}</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {(packages || []).map(pkg => (
                  <button
                    ...
                    className={`p-10 rounded-[3rem] border-4 text-left transition-all duration-300 relative group ${
                  ...
                  >
                    ...
                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2">{pkg?.durationDays || 0} Days Duration</p>
                    <p className="text-2xl font-black text-slate-800 tracking-tighter mb-6 leading-none">{pkg?.name || 'Generic Package'}</p>
                    <div className="flex items-baseline space-x-1">
                      <span className="text-4xl font-black text-blue-600">₹{pkg?.price || 0}</span>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="p-12 bg-slate-50 border-t border-slate-100 rounded-b-[4rem] flex flex-col sm:flex-row justify-end items-center gap-8 flex-shrink-0">
              <button onClick={() => setIsRechargeModalOpen(false)} className="px-12 py-6 font-black text-slate-400 uppercase tracking-[0.4em] text-[13px] hover:text-slate-800 transition-colors">Abort</button>
              <button 
                disabled={!selectedPkgId}
                onClick={() => {
                  if (selectedUser) onRecharge(selectedUser.id, selectedPkgId);
                  setIsRechargeModalOpen(false);
                }}
                className="px-32 py-8 bg-blue-600 text-white rounded-[2.5rem] shadow-[0_30px_60px_rgba(37,99,235,0.4)] uppercase tracking-[0.3em] text-[13px] font-black transition-all hover:bg-blue-700 active:scale-95 disabled:opacity-50"
              >
                Authorize & Provision
              </button>
            </div>
